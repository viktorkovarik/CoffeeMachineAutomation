				...
				if ((xmlHttp.readyState==4) && (xmlHttp.status==200)) { //process is completed and http status is OK
					...
					for ( var i in pole ) {
						...
						//if (Date.parse(pole[i].timestamp) > pole_time_old) {
							console.log(pole[i]);
							jednotka = "";
							if (pole[i].sensorType == "Temperature"){
								jednotka = " °C";
							}
							else if (pole[i].sensorType == "Humidity"){
								jednotka = " %";
							}
							else if (pole[i].sensorType == "Proximity"){
								jednotka = " lux";
							}
							else if (pole[i].sensorType == "Pressure"){
								jednotka = " hPa";
							}
					
							pom += '<tr>';
							
							pom += '<td>'+pole[i].uniqueID+'</td>';
							pom += '<td>'+pole[i].sensorType+'</td>';
							pom += '<td>'+pole[i].value+jednotka+'</td>';
							pom += '<td>'+pole[i].timestamp+'</td>';
							...
							if ( pole_time_old != 0) {
								pole_time_old = Date.parse(pole[i].timestamp);
							}
